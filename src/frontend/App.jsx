...
const API_BASE_URL = stripTrailingSlash(import.meta.env.VITE_API_BASE_URL || "");
...
`;
  ...
  const dbNormalizedStatus = serverStatus.db?.ok ? "ok" : "offline";
  const dbStatusLabel = serverStatus.db?.ok ? "Connexion DB : OK" : "Connexion DB : indisponible";
  const dbStatusIndicatorClass = `status-indicator status-indicator--${dbNormalizedStatus}`;
  ...
  useEffect(() => {
    ...
    const fetchServerStatus = async () => {
      try {
        ...
        const dbPayload =
          data?.db && typeof data.db === "object"
            ? { ok: Boolean(data.db.ok), error: data.db.error || "" }
            : { ok: false };
        setServerStatus({
          status,
  ...
  const footer = (
    <footer className="app-footer">
      <div className="footer-item">
        <span className={dbStatusIndicatorClass} aria-hidden="true" />
        <span>{dbStatusLabel}</span>
      </div>
      <div className="footer-item">Version v{appVersionLabel}</div>
    </footer>
  );
        ...
        <section className="panel class-summary-panel">
          ...
          {visibleSummaryRows.length === 0 ? (
            ...
            <div className="class-summary-table-panel">
              <table className="class-summary-table class-summary-with-methods">
                <thead>
                  <tr>
                ...
                <tbody>
                  {visibleSummaryRows.map((row) => (
                    <tr key={row.id}>
                      ...
                      <td className={`class-summary-note ${row.noteClass}`}>
                        {row.noteLabel}
                      </td>
                      {TASK_EVALUATION_METHODS.map((method) => (
                        <td
                        ...
                        >
                          {row.nokEvaluationMethods
                            ? row.nokEvaluationMethods.includes(method.value)
                              ? "X"
                              : ""
                            : ""}
                        </td>
